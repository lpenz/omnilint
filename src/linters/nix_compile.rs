...
impl NixNixInstantiate {
    ...
    fn parse_line(filename: &Path, line: &str) -> Option<Entry> {
        ...
        let marker = format!("at {fname}:");
        let idx = line.find(&marker)?;
        let after = &line[idx + "at ".len()..];
        let rest = after.strip_prefix(fname)?;
        let rest = rest.strip_prefix(':')?;
        let (line_str, rest) = rest.split_once(':')?;
        let line_num: u32 = line_str.parse().ok()?;
        let col_str = rest.split_once(':').map_or(rest, |(before, _)| before);
        let col_num: u32 = col_str.parse().ok()?;
        let msg = if idx > 0 {
            line[..idx].trim().trim_end_matches(',')
        } else {
            "syntax error"
        };
